...
class InputTracker:
    ...
    def save(self, file):
        tracker_saved = InputTrackerSaveVersion(self.window_size, self.num_hashes_keep, self.round, self.salt,
                                                self.hash_dict, self.input_idx)
        pickle.dump(tracker_saved, open(file, "wb"))

    @staticmethod
    def load(file):
        tracker_saved = pickle.load(open(file, "rb"))
        tracker = InputTracker(None, tracker_saved.window_size, tracker_saved.num_hashes_keep,
                               round=tracker_saved.round, workers=5, salt=tracker_saved.salt)
